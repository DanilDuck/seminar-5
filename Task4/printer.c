...
int main() {
    ...
    struct AST* plus = add(_999_, _728_);
    print(stdout, plus);
    printf("\n");
    struct AST* _3_ = lit(3);
    struct AST* _5_ = lit(5);
    struct AST* p3_5 = add(_3_, _5_);
    struct AST* _9_ = lit(9);
    struct AST* _7_ = lit(7);
    struct AST* d9_7 = div(_9_, _7_);
    struct AST* mult = mul(p3_5, d9_7);
    print(stdout, mult);
    return 0;
}
